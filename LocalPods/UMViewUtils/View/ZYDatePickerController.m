...
- (instancetype)initWithMessage:(NSString *)message mode:(UIDatePickerMode)mode dateFormat:(NSString *)dateFormat handler:(void (^)(NSString *))handler {
    ...
    if (self) {
        ...
        [self.confirmButton setTitle:NSLocalizedString(@"OK",@"")  forState:UIControlStateNormal];
    ...
    [self.cancelButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (@available(iOS 11.0, *)) {
            if ([UIScreen mainScreen].bounds.size.width == 375.f && [UIScreen mainScreen].bounds.size.height == 812.f) {
                make.bottom.equalTo(self.view.mas_bottom);//mas_safeAreaLayoutGuideBottom
            } else {
                make.bottom.equalTo(self.view.mas_bottom).offset(-10).priorityHigh();//mas_safeAreaLayoutGuideBottom
            }
            make.bottom.equalTo(self.view.mas_bottom);//mas_safeAreaLayoutGuideBottom
        } else {
            // Fallback on earlier versions
            make.bottom.equalTo(self.view.mas_bottom).offset(-10).priorityHigh();
        }
        make.height.mas_offset(57).priorityHigh();
    }];
...
- (void)setDateAtString:(NSString *)sDate{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    dateFormatter.dateFormat = self.dateFormat ?: @"yyyy-MM-dd";
    self.datePicker.date = [dateFormatter dateFromString:sDate];
}
